...
for file_index, file_name in enumerate(os.listdir(data_dir)):
        if file_index < 1:
            ...
            for key, item in sold:
                data = sold.get_group(key);
                #data.plot(kind='scatter',x='predict_replies',y='predict_views')
                ax1.scatter(data['predict_replies'],data['predict_views'],label=key)
