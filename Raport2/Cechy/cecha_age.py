...
outputDir = "D:/Biblioteki/Dokumenty (D)/Studia/ReportNinja2/OLXOUTPUT/age"
snapshotDate = datetime.date(2016,11,1)
...
def convertAge(txt):
    if txt.strip() is None or txt != re.sub('[a-zA-Z]+', '', txt):
        return ""
    else:
        age = 0
        creationDate = parse(txt).date()
        age = snapshotDate - creationDate
        return age.days

converters = {8: convertAge}
categories = []
grouped = []
mean_sold = [];
max_sold = [];
